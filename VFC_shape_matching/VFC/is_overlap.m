...
% IS_OVERLAP computes whether two conics overlap, if overlap, return 1,
...
% conic is a circle with centre T2 and radius R2.
% Then the two conics can be described as follows:
%                  [x]                          [   1       0          -t1(1)         ]               [   1       0           -t2(1)        ]
% [x, y, 1] * Ci * [y] = 0,     where C1=invHT* [   0       1          -t1(2)         ] * invH,    C2=[   0       1           -t2(2)        ].
%                  [1]                          [ -t1(1)  -t1(2)  t1(1)^2+t1(2)^2-r1^2]               [ -t2(1)  -t2(2)  t2(1)^2+t2(2)^2-r2^2]
%   We determine a pair of conics overlaps by first checking whether one
%   conic centre is inside the other conic, if so, they overlap, return;
%   otherwise, we compute if the two conics intersect.
