function [p,C]=LPM_cosF(neighborX,neighborY,lambda,vec,d2,tau,K, r, d_gc, mu)
%% *************************************
% Multi-scale.  
...
%% global consensus distance calculation
[clustCent,data2cluster,cluster2dataCell,numClust,pro] = MeanShiftCluster(d_gc,r);
alpha=pro(data2cluster);
alpha=1./alpha;
distance=1-exp(-alpha.*d_gc);

p = ((C./M)+ mu*distance) <= lambda.*ones(1,L);%C/M  0.2*distance(2,:)  
C = (C./M)+ mu*distance;
