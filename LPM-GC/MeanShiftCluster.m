function [clustCent,data2cluster,cluster2dataCell,numClust,pro] = MeanShiftCluster(dataPts,bandWidth,plotFlag)
...
%         sqDistToAll = sum((repmat(myMean,1,numPts) - dataPts).^2);    %dist squared from mean to all points still active
        ...
        sqDistToAll = (repmat(myMean,1,numPts) - dataPts).^2;
%         sqDistToAll=sqDistToAll(2,:);
        ...
        if plotFlag
            ...
            if numDim == 1
                plot(1:numPts,dataPts,'.')
                plot(initPtInds(myMembers),dataPts(myMembers),'ys')
                plot(myMean(1),myMean(2),'go')
                plot(myOldMean(1),myOldMean(2),'rd')
                pause(.1)
            end
...
for i=1:numClust
   number= size(cluster2dataCell{i,1},2);
   pro(i)= number/numPts;
end
